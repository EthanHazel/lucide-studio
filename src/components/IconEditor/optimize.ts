            ...
            if (data.children[i].attributes.d) {
              if (
                ...
                data.children[i2].attributes.d = Commander.pathToString(
                  a.command.segments.concat(
                    b.command.reverse().segments.slice(1),
                  ) as any,
                );
                // @ts-ignore
            ...
            if (!isDistanceSmaller(a.start, a.end, maxDistance)) {
              if (isDistanceSmaller(a.end, b.end, maxDistance)) {
                ...
                data.children[i2].attributes.d = Commander.pathToString(
                  b.command
                    .reverse()
                    .segments.concat(a.command.segments.slice(1)) as any,
                );
                // @ts-ignore
                data.children[i].name = undefined;
                i = 0;
...
const getLinesAndPoints = (children: INode[]) => {
  ...
  for (let i = 0; i < children.length; i++) {
    const command = commander(
      children[i].name === "path"
        ? children[i].attributes.d
        : toPath(children[i]),
    );
    let prevPoint: Point;
    let nextPoint: Point;
  ...
  for (let i = 0; i < data.children.length; i++) {
    if (data.children[i].name === "path") {
      ...
      if (command.getTotalLength() <= 0.1) {
        ...
        if (
          lines.some(
            (line) => line.id !== i && isPointInLine(line, { x, y }),
          ) ||
          points.some(
            (point) =>
              point.id !== i && isDistanceSmaller(point, { x, y }, 0.1),
          )
        ) {
          // @ts-ignore
...
const removeTinySegments = (svg: string) => {
  ...
  for (let i = 0; i < data.children.length; i++) {
    if (data.children[i].name === "path") {
      ...
      const command = commander(data.children[i].attributes.d);
      for (let j = 1; j < command.segments.length - 1; j++) {
        if (
          isDistanceSmaller(
            pathData.commands[j] as Point,
            pathData.commands[j - 1] as Point,
            0.05,
          )
        ) {
    ...
    if (data.children[i].name === "path") {
      ...
      for (let i2 = 0; i2 < command.segments.length; i2++) {
        ...
        if (
          ...
          command.segments[i2] = [
            "M",
            lines[i3].nextPoint.x,
            lines[i3].nextPoint.y,
          ];
          lines.splice(i3, 1);
...
const getCircle = (segments: PathSegment[]): INode | undefined => {
  ...
  for (let j = 0; j < segments.length; j++) {
    ...
    if (segment[0] === "M") {
      ...
      if (
        prevCenters.at(-1) &&
        // @ts-ignore
        !isDistanceSmaller(center, prevCenters.at(-1), 0.1)
      ) {
        return undefined;
      }
  ...
  return {
    ...
    attributes: {
      cx:
        prevCenters.reduce((acc, { x }) => acc + x, 0) / prevCenters.length +
        "",
      cy:
        prevCenters.reduce((acc, { y }) => acc + y, 0) / prevCenters.length +
        "",
      r:
        prevCenters.reduce((acc, { rx, ry }) => acc + rx + ry, 0) /
          (prevCenters.length * 2) +
        "",
    },
  };
};
...
const getRect = (segments: PathSegment[]): INode | undefined => {
  ...
  for (let i = 0; i < rest.length; i++) {
    ...
    if (segment[0] === "L") {
      if (
        // @ts-ignore
        Math.abs(segment[1] - prevPoint.x) > 0.1 &&
        // @ts-ignore
        Math.abs(segment[2] - prevPoint.y) > 0.1
      ) {
        return undefined;
      }
  ...
  if (
    points.length !== 4 ||
    // @ts-ignore
    !isDistanceSmaller(prevPoint, { x: start[1], y: start[2] }, 0.1)
  ) {
    return undefined;
  }
...
const getArcFromCurve = (
  ...
  const c1 = { x: segment[1] - prevPoint.x, y: segment[2] - prevPoint.y };
  const c2 = { x: segment[3] - segment[5], y: segment[4] - segment[6] };
  if (
...
const segmentsToArc = (svg: string) => {
  ...
  data.children.forEach((c, idx) => {
    if (c.name === "path") {
      const command = commander(
        new SVGPathData(c.attributes.d)
...
const optimizeRect = (svg: string) => {
  ...
  for (let i = 0; i < data.children.length; i++) {
    if (data.children[i].name === "rect") {
      ...
      if (
        (data.children[i].attributes.ry !== undefined ? ry : rx) ===
          width / 2 &&
        rx === height / 2
      ) {
        data.children[i] = {
...
const smartClose = (svg: string) => {
  ...
  for (let i = 0; i < data.children.length; i++) {
    if (data.children[i].name === "path") {
      const { command, start, end } = getSegmentInfo(
...
const mergeArcs = (svg: string) => {
  ...
  const notArcs = data.children.flatMap((node, idx) => {
    ...
    const segments = paths.filter(
      ({ c }) => c.id === idx && c.type !== SVGPathData.ARC,
    );
...
export default memoize((value: string) =>
  runOptimizations(runOptimizations(value)),
);
